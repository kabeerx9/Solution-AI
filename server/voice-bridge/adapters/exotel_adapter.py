...
"""
    ...
    async def handle_inbound_call(
        ...
        logger.info(f"📞 Incoming Exotel call: {call_sid} from {mask_phone_number(from_number)}")
        ...
        conversation = StreamingConversation(
            ...
            agent=ChatGPTAgent(
                self.create_agent_config(context)
            ),
    ...
    def get_webhook_routes(self):
        ...
        async def exotel_incoming(request: Request):
            """Handle incoming Exotel call webhook"""
        ...
        async def exotel_stream(websocket: WebSocket, call_sid: str):
            ...
            try:
                ...
                if call_sid in self.active_calls:
                    start_time = self.active_calls[call_sid].get("start_time", time.time())
